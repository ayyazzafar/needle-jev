...
// TYPESAFE_API_KEY calls Jev directly; otherwise fall back to Vercel AI Gateway.
export function getProvider(env = process.env) {
  ...
  let key = env.AI_GATEWAY_API_KEY?.trim() || "";
  if (!key && env.AI_GATEWAY_KEY_FILE) {
    try {
      key = readFileSync(env.AI_GATEWAY_KEY_FILE, "utf8").trim();
    } catch {}
  }
  return { direct: false, key, model: MODEL };
}
...
export function makePayload({ query, blocks }, direct = false) {
  const questions = {};
  for (const block of blocks) {
    questions[block.id] = {
      type: direct ? "noul" : "boolean",
  ...
  return {
    model: direct ? DIRECT_MODEL : MODEL,
...
export function parseAnswers(data, blocks) {
  ...
  return blocks
    .map((b) => {
      const p = answers[b.id]?.probability ?? answers[b.id]?.noul;
...
export async function search(
  body,
  { fetchImpl = fetch, key, direct = false } = {},
) {
  const input = validate(body);
  const provider =
    key !== undefined
      ? { key, direct, model: direct ? DIRECT_MODEL : MODEL }
      : getProvider();
  if (!provider.key)
    throw new SearchError(
      "Search needs a TypeSafe or AI Gateway key on the server.",
      503,
    );
  const name = provider.direct ? "TypeSafe" : "AI Gateway";
  const start = performance.now();
  let response;
  try {
    response = await fetchImpl(provider.direct ? DIRECT_URL : GATEWAY_URL, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${provider.key}`,
        "Content-Type": "application/json",
      },
      body: JSON.stringify(makePayload(input, provider.direct)),
  ...
  if (!response.ok) {
    const messages = {
      401: `The server’s ${name} key was rejected.`,
      402: `${name} credits or account verification are required.`,
      403: `This ${name} account cannot access Jev.`,
      429: "Too many searches. Give it a moment and try again.",
    };
    throw new SearchError(
      messages[response.status] ||
        `${name} could not complete this search. Please try again.`,
  ...
  try {
    ...
    throw new SearchError(`${name} returned an unreadable response.`, 502);
  }
