...
> **About this version.** This is a modified copy of [Needle](https://github.com/Shubhamsaboo/awesome-llm-apps/tree/main/advanced_llm_apps/needle) from Shubham Saboo's [awesome-llm-apps](https://github.com/Shubhamsaboo/awesome-llm-apps), redistributed under the [Apache-2.0 license](LICENSE). All credit for the original app goes to its authors. This project is not affiliated with or endorsed by them, TypeSafe AI or Vercel.
>
> **What changed:** Needle can now call Jev **directly with your own TypeSafe API key** (`TYPESAFE_API_KEY`), so you no longer need a Vercel AI Gateway account. The original AI Gateway option still works. Changed files carry a note at the top. See [CHANGES.md](CHANGES.md).
...
The backend calls Jev directly through the [TypeSafe API](https://docs.typesafe.ai/api) when `TYPESAFE_API_KEY` is set. Otherwise it accesses `typesafe-ai/jev` through [Vercel AI Gateway's evaluation API](https://vercel.com/docs/ai-gateway/modalities/evaluation). Jev selects source sentences rather than generating an answer. Results with relevance scores of at least `0.58` are included; this is a ranking threshold, not a guarantee that every relevant passage was found.

## How to Get Started

Requires **Node.js 22.12+**, npm, and **one** of these keys:

- **TypeSafe API key (simplest):** create one in the [TypeSafe console](https://console.typesafe.ai/keys).
- **Vercel AI Gateway key:** from the [Vercel AI Gateway dashboard](https://vercel.com/dashboard/ai-gateway), with access to `typesafe-ai/jev` and credits.

An OpenAI key is not a substitute.

```sh
git clone https://github.com/ayyazzafar/needle-jev.git
cd needle-jev
npm ci
cp .env.example .env
```
...
Never put your TypeSafe or Gateway key in the extension, a `VITE_` environment variable, a screenshot, or a committed file. `.env` is ignored by Git. The optional `AI_GATEWAY_KEY_FILE` setting can read an existing local key file instead; ordinary users only need `.env`.
...
2. Choose **Load unpacked** and select the **`needle-jev/extension/`** folder. This is the folder containing `manifest.json`. Do not select the repository root or a ZIP file.
...
| API key or credits error        | Check `TYPESAFE_API_KEY` (or `AI_GATEWAY_API_KEY`) in `.env`, your account's access and credits, then restart the server. |
...
| --------------------- | -------------------------------------------------------------- | ----------------------------------------------------------------------- |
| `TYPESAFE_API_KEY`    | Backend `.env`                                                 | Calls Jev directly; takes priority; never goes into the extension       |
...
1. Import your fork into Vercel. Leave **Root Directory** as the repository root and set Framework Preset to **Vite**. The included `vercel.json` specifies the build output and API function duration.
2. Add `TYPESAFE_API_KEY` (or `AI_GATEWAY_API_KEY`) and a randomly generated `NEEDLE_ACCESS_TOKEN` to the project’s environment variables. For example, generate the access token with:
...
- Search sends **all captured passages plus your query**, not just the highlighted result, to the configured backend and to TypeSafe (directly or through AI Gateway). Use it only on pages you are comfortable sending to those services.
...
Licensed under the [Apache-2.0 license](LICENSE). Original work © its authors in [awesome-llm-apps](https://github.com/Shubhamsaboo/awesome-llm-apps); modifications © 2026 Ayyaz Zafar, under the same licence.
